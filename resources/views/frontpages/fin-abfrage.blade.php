...
/* Blau, solange fokussiert … */
#finContainer:focus-within {
  border-color: var(--primary);
  box-shadow: 0 0 0 4px rgba(0, 123, 255, 0.1);
}

/* … aber wenn valid, immer grün – auch im Fokus */
#finContainer.valid,
#finContainer.valid:focus-within {
  border-color: var(--secondary) !important;
  box-shadow: 0 0 0 4px rgba(40, 152, 227, 0.0); /* ggf. Fokus-Ring deaktivieren */
}

#finContainer.valid,
#finContainer.valid:focus-within,
#finContainer.valid.highlight-border {
  border-color: var(--secondary) !important;
...
">
    ...
    <p style="font-size:1.1rem; color:#444; max-width:750px; margin:0 auto">
...
<div class="pt-4" style="display:flex; flex-direction:column; gap:1rem; align-items:center; max-width:450px; margin:0 auto 1.5rem;">
...
<a href="#fin-model" data-bs-target="#fin-model" data-bs-toggle="modal">
  ...
  Jetzt FIN überprüfen <span class="arrow">→</span>
    ...
    <div class="pb-1 pt-1" style="display: flex; justify-content: center; align-items: center; font-size: 1rem; color: #444; gap: 6px;">
...
<div style="max-width: 450px; margin: 16px auto; padding: 14px 20px; 
            background: #f5faffff; border: 1px solid #d6e6f9; 
            border-radius: 12px; box-shadow: 0 2px 6px rgba(0,0,0,0.04); 
            text-align: center; font-size: 1rem; color: #222; line-height: 1.5;">
  🎉 Exklusiv durch unsere Partnerschaft mit <strong>carVertical</strong>:
  <span style="font-weight: 600; color: #d32f2f;">20 % Rabatt</span> auf jede FIN-Abfrage!
</div>


    <section class="pt-3" style="display:flex; align-items:center; justify-content:center; gap:0.5rem; font-size:1rem; font-weight:500; color:#333;">
...
input.addEventListener("input", function () {
  const fin = input.value.trim().toUpperCase();
  input.value = fin; // bleibt upper-case

  if (fin.length === 17) {
    container.classList.add("valid");        // => grün dominiert
    checkIcon.style.display = "inline";
  } else {
    container.classList.remove("valid");     // => normal/fokus wird blau
...
function scrollAndHighlightFinInput() {
  ...
  setTimeout(() => {
    ...
    container.classList.remove("highlight-border");
    void container.offsetWidth; // reflow, um Animation zu resetten
    container.classList.add("highlight-border");

    // Nach Ende der Animation die Klasse wieder entfernen
    const onAnimEnd = (e) => {
      if (e.animationName === "pulse-highlight") {
        container.classList.remove("highlight-border");
  ...
  <div style="max-width:1100px; margin:auto; display:flex; flex-wrap:wrap; gap:2rem; align-items:center;">
    ...
    <div style="flex:1 1 400px;">
      ...
      <button onclick="scrollAndHighlightFinInput()" class="scroll-button">
        Jetzt FIN prüfen und 20 % sparen <span style="font-size:1.1rem;">→</span>
      </button>
    ...
    <div style="display: flex; flex-direction: row; justify-content: space-between; gap: 40px; flex-wrap: wrap;">
      ...
      <div class="pt-3 pt-sm-0" style="flex: 1; min-width: 280px; text-align: center;">
        ...
        <div style="display: flex; justify-content: center; align-items: center; gap: 10px; margin-bottom: 10px;">
          ...
          <div style="font-size: 20px; font-weight: bold; color: var(--primary);">FIN wird überprüft</div>
        </div>
...
<section style="padding:4rem 2rem; background:#f8fafc; overflow:hidden;">
  ...
  <li><i class="fas fa-file-alt" style="color:var(--primary); margin-right:0.5rem;"></i> Erhalte detaillierte, geprüfte und vertrauenswürdige Fahrzeuginfos.</li>
    ...
    <div class="trust-card">
      ...
      <p class="subline">Kunden vertrauen auf die Expertise und den Service von carVertical.</p>
    </div>
  </div>
</section>
