    ...
    function doScan(url) {
      ...
      .then(function(data) {
        if (data.success) {
          ...
          // Only show preview for autoscout24 — mobile.de results may not match the exact listing
          if (data.platform !== 'mobile' && (data.brand || data.model || data.make_year || data.mileage || data.price)) {
            showPreview(data);
          }
        } else {
