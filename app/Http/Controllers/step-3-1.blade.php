        ...
        <form id="bookingForm" action="{{ route('booking.step1') }}" method="POST" class="panel">
          ...
          <div class="panel-body">
            ...
            <div id="scanResult" style="display:none;background:#f0f9f4;border:1px solid #c3e6cb;border-radius:10px;padding:12px 14px;margin-bottom:10px;">
              <div style="font-weight:600;margin-bottom:6px;" id="previewTitle"></div>
              <div style="display:flex;gap:10px;flex-wrap:wrap;font-size:.9rem;color:#555;">
  ...
  (function(){
    ...
    function doScan(url) {
      ...
      .then(function(data) {
        if (data.success) {
          ...
          setHidden('listing_scrape_status',  data.status         || '');
          if (data.brand || data.model) {
            var vm = document.getElementById('vehicle_make_model');
            if (vm && !vm.value) vm.value = [data.brand, data.model].filter(Boolean).join(' ');
          }
          if (data.brand || data.model || data.make_year || data.mileage || data.price) {
            showPreview(data);
          }
        } else {
    ...
    if (adInput) {
      adInput.addEventListener('input', function() {
        ...
        debounce = setTimeout(function(){ doScan(url); }, 800);
      });
      // Also trigger on paste immediately after a short delay
